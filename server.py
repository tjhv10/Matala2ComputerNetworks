...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        server_socket.bind((host, port)) # assign an IP address and a port number to the socket
        server_socket.listen() # start listening for requests from clients
        ...
        while True:
            try:
                ...
                # * Fill in start (2) 
                client_socket, address = server_socket.accept() # Accept an incoming connection request from the TCP client.
                 # * Fill in end (2)
