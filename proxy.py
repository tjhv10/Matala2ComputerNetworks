...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        proxy_socket.bind((proxy_address[0], proxy_address[1]))  # assign an IP address and a port number to the proxy socket
        proxy_socket.listen()  # start listening for requests from clients
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                client_socket, client_address = proxy_socket.accept() # Accept an incoming connection request from the TCP client.
                 # * Fill in end (2)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # * Fill in start(3)
            data = client_socket.recv(8180)   #Receive data from client
            # * Fill in end (3)
